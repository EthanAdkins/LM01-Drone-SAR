...
class drone_env(gymnasium.Env):
    def __init__(self, LOCAL_IP, Name):
        super(drone_env, self).__init__()
        global droneName 
        droneName = Name
        #  -- set drone client --
        self.drone = airsim.MultirotorClient(LOCAL_IP) 
        self.drone.confirmConnection()
        
        self.max_timestep= 500 

        self.goal_name = "BrianMasterAi13821" # this is the target (person) in World environment
    ...
    def doAction(self, action):
        ...
        if rotate == 0:
            quad_vel = self.drone.getMultirotorState(vehicle_name=droneName).kinematics_estimated.linear_velocity
            self.drone.moveByVelocityAsync(
                quad_vel.x_val + quad_offset[0],
                quad_vel.y_val + quad_offset[1],
                quad_vel.z_val + quad_offset[2],
            ...
            self.drone.rotateByYawRateAsync(quad_offset, .5, vehicle_name=droneName).join()
        # self.drone.stopRecording()
    ...
    def startFlight(self):
        ...
        self.drone.enableApiControl(True, vehicle_name = droneName)
        self.drone.armDisarm(True,vehicle_name = droneName) 
        ...
        # self.drone.startRecording()
        self.drone.moveByVelocityAsync(0, 0, 0, 10, vehicle_name=droneName).join()
        # self.drone.stopRecording()
    ...
    def getImageObs(self):
        ...
        image_request = self.drone.simGetImages([airsim.ImageRequest("front_center", airsim.ImageType.DepthPerspective, True, False), 
                                                    airsim.ImageRequest("front_center", airsim.ImageType.Scene, False, False)], vehicle_name=droneName)
    ...
    def getObservation(self, chosenAction):
        ...
        self.drone_state = self.drone.getMultirotorState(vehicle_name=droneName)

        kinematics = self.drone.getMultirotorState(vehicle_name=droneName).kinematics_estimated
        self.info["collision"] = self.drone.simGetCollisionInfo(vehicle_name=droneName).has_collided #check if drone has collided
        ...
        get_alt = self.drone.getDistanceSensorData("Distance", vehicle_name=droneName).distance
    ...
    def disconnect(self):
        self.drone.enableApiControl(False, vehicle_name=droneName)
        self.drone.armDisarm(False, vehicle_name=droneName)
