...
class  AirSimDroneEnvV1(AirSimEnv):
    def __init__(self, ip_address, step_length, image_shape):
        ...
        self.observation_space = spaces.Dict({
            "prev_position": spaces.Box(low=np.array([-np.inf for _ in range(3)]), 
                                    high=np.array([np.inf for _ in range(3)]),
                                    dtype=np.float64),
            "position": spaces.Box(low=np.array([-np.inf for _ in range(3)]), 
                                    high=np.array([np.inf for _ in range(3)]),
                                    dtype=np.float64),
            "prev_dist": spaces.Box(low=np.array([-np.inf for _ in range(3)]), 
                                            high=np.array([np.inf for _ in range(3)]),
                                            dtype=np.float64),
            "curr_dist": spaces.Box(low=np.array([-np.inf for _ in range(3)]), 
                                            high=np.array([np.inf for _ in range(3)]),
                                            dtype=np.float64),
            "velocity": spaces.Box(low=np.array([-np.inf for _ in range(3)]), 
                                    high=np.array([np.inf for _ in range(3)]),
                                    dtype=np.float64),
            "processed_lidar": spaces.Box(low=np.array([-np.inf for _ in range(4)]), 
    ...
    def _setup_flight(self):
        ...
        pos_x = self.origin.x_val
        pos_y = self.origin.y_val
        pos_z = self.origin.z_val
        coords = np.array([pos_x,pos_y,pos_z], dtype=np.float64)

        # self.origin_dist_to_target = self.calc_dist(self.origin, self.get_destination())
        self.origin_dist_to_target = self.calc_dist(coords, self.get_destination())
    ...
    def get_destination(self):
        ...
        # dis_x = 70.68778991699219
        # dis_y = 198.01834106445312
        # dis_z = -17.886749267578125

        self.goal_name = "Character_1"
        goal_pos = self.drone.simGetObjectPose(self.goal_name).position
        dis_x = goal_pos.x_val
        dis_y = goal_pos.y_val
        dis_z = goal_pos.z_val
        destination = np.array([dis_x, dis_y, dis_z], dtype=np.float64)
    ...
    def lidar_processing(self):
        lidar_results = np.zeros(4)
        lidarData = self.drone.getLidarData(lidar_name="LidarSensor1", vehicle_name= "Drone1")
        if(len(lidarData.point_cloud)):
            lidar_results[0] = 1
        else:
            lidar_results[0] = 0

        lidarData = self.drone.getLidarData(lidar_name="LidarSensor2", vehicle_name= "Drone1")
        if(len(lidarData.point_cloud)):
            lidar_results[1] = 1
        else:
            lidar_results[1] = 0    

        lidarData = self.drone.getLidarData(lidar_name="LidarSensor3", vehicle_name= "Drone1")
        if(len(lidarData.point_cloud)):
            lidar_results[2] = 1
        else:
            lidar_results[2] = 0

        lidarData = self.drone.getLidarData(lidar_name="LidarSensor4", vehicle_name= "Drone1")
        if(len(lidarData.point_cloud)):
            lidar_results[3] = 1
        else:
            lidar_results[3] = 0
    ...
    def _get_obs(self):
        ...
        pos_x = self.drone_state.kinematics_estimated.position.x_val
        pos_y = self.drone_state.kinematics_estimated.position.y_val
        pos_z = self.drone_state.kinematics_estimated.position.z_val
        # self.state["position"] = self.drone_state.kinematics_estimated.position
        self.state["position"] = np.array([pos_x,pos_y,pos_z], dtype=np.float64)

        vel_x = self.drone_state.kinematics_estimated.linear_velocity.x_val
        vel_y = self.drone_state.kinematics_estimated.linear_velocity.y_val
        vel_z = self.drone_state.kinematics_estimated.linear_velocity.z_val
        # self.state["velocity"] = self.drone_state.kinematics_estimated.linear_velocity
        self.state["velocity"] = np.array([vel_x,vel_y,vel_z], dtype=np.float64)
    ...
    def calc_dist(self, pointA, pointB):
        ...
        euclidean_distance = np.linalg.norm(pointA-pointB)
    ...
    def _compute_reward(self):
        ...
        pos_z = self.origin.z_val
        coords = np.array([pos_x,pos_y,pos_z], dtype=np.float64)
        origin_dist_to_target = self.calc_dist(target_l, coords)
