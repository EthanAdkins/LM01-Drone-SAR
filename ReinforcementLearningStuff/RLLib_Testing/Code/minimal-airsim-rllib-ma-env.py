...
if __name__ == "__main__":
    ...
    def env_creator(env_config):
        ...
        max_conc = 2
        return ExternalAirSimMultiAgent(env_config, a, o, max_conc)
    ...
    # config = config.rollouts(num_rollout_workers=0 ,enable_connectors=False)
    # PPOTrainer = get_trainer_class("PPO")
    # agent = PPOTrainer(config, env="ext-ma-airsim")
    # iterations = 2
    algo = (
        ppo.PPOConfig()
        .rollouts(num_rollout_workers=0, enable_connectors=False)
        .environment(env="ext-ma-airsim")
        .build()
    )
    iterations = 2
    # Training loop
    ...
    tune.run("PPO",stop=stop, config=algo, verbose=1)
    ray.shutdown()
